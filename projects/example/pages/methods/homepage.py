...
class HomePage(BasePage):
    ...
    def microsoft_azure_handler(self):
        ...
        if email is None:
            email = self._constants.email
        password = os.getenv("password")
        if password is None:
            password = self._constants.password
        self.web_utils.wait_until_element_visible(self._locators.email_text_box, 10)
        email_textbox_obj = self.web_utils.get_object(self._locators.email_text_box, "Email textbox")
        submit_obj = self.web_utils.get_object(self._locators.submit_button, "Submit Button")
        email_textbox_obj.send_keys(email)
        self.web_utils.left_click(submit_obj)
        self.web_utils.wait_until_element_visible(self._locators.password_text_box, 10)
        password_textbox_obj = self.web_utils.get_object(self._locators.password_text_box, "Password Text Box")
        password_textbox_obj.send_keys(password)
        submit_obj = self.web_utils.get_object(self._locators.submit_button, "Submit Button")
        self.web_utils.left_click(submit_obj)

    def enter_text_in_search_text_box(self, text_to_enter):
        search_input_box = self.web_utils.get_object(self._locators.search_input_box, "Search Input Box")
        search_input_box.send_keys(text_to_enter)

    def click_on_the_search_button(self):
        search_button = self.web_utils.get_object(self._locators.search_button, "Search Button")
        self.web_utils.left_click(search_button)
