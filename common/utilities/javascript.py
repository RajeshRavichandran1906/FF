...
class JavaScript:
    ...
    def get_elements_text(self, elements_object):
        """
        Description: This method used to find given css elements and return elements text as list
        ...
        elements_text = self._driver.execute_script(script, elements_object)
        return elements_text

    def get_element_text(self, element_obj):
        """
        Description: This method is used to get the text of single element
        """
        script = "return arguments[0].innerText"
        element_text = self._driver.execute_script(script, element_obj)
        return element_text

    def find_elements_by_text(self, elements_object, text_to_find):
        """
        Description: This method used to find elements by given text. It will return element as list if given text found
        """
        ...
        found_elements = self._driver.execute_script(script, elements_object)
        return found_elements

    def scroll_element(self, scrollable_element_css, scroll_offset, wait_time=0):
        """
        Description: This method used to scroll the element based on scroll_offset value. element should be have scrollable
        """
    ...
    def get_element_property_value(self, element_object, property_name):
        """
        Description: This method used to get element property like scrollTop, textContent and etc
        """
        script = "return arguments[0]." + property_name
        property_value = self._driver.execute_script(script, element_object)
        return property_value

    def check_scroll_is_completed(self, scrollable_element_css):
        """
        Description: This method used to check whether scroll is completed which means scroll bar reached bottom of the element
        """
        scrollable_element_object = self._driver.find_element_by_css_selector(scrollable_element_css)
        script = "return arguments[0].offsetHeight + arguments[0].scrollTop == arguments[0].scrollHeight"
        status = self._driver.execute_script(script, scrollable_element_object)
        return status

    def get_scroll_offsetTop(self, element_object):
        """
        Description: This method used to get element scroll offsetTop value
        """
    ...
    def scrollTop(self, scrollable_element_css, scroll_to_element_obj, wait_time=2):
        """
        Description: This function used to scroll the given scrollable element based of scroll_to_element offsetTop value
        """
        scrolltop_value = self._driver.execute_script('return arguments[0].offsetTop', scroll_to_element_obj)
        scroll_script_syntax = 'document.querySelector("{0}").scrollTop={1}'.format(scrollable_element_css,
                                                                                    scrolltop_value)
        self._driver.execute_script(scroll_script_syntax)
        time.sleep(wait_time)
    ...
    def scrollLeft(self, scrollable_element_object, scroll_to_element_obj, wait_time=1):
        ...
        self._driver.execute_script(scroll_script_syntax, scrollable_element_object)
        time.sleep(wait_time)

    def find_element_index_by_text(self, elements_object, text_to_find, index_to_start=0):
        """
        Description: This method used to find element index by given text. It will return element index if given text match otherwise None will return
        ...
        found_elements = self._driver.execute_script(script, elements_object, index_to_start)
        return found_elements

    def find_all_index_of_element_by_text(self, elements_object, text_to_find, index_to_start=0):
        """
        Description: This method used to find element index by given text. It will return element index if given text match otherwise empty will return
        :arg - index_to_start = From which row start to find element text
        """
        ...
        found_index_list = self._driver.execute_script(script, elements_object, index_to_start)
        return found_index_list

    def scrollIntoView(self, element_obj, wait_time=2):
        """
        Description: If the element is fully within the visible area of the viewport, it does nothing. Otherwise,
    ...
    def get_element_before_style_properties(self, element_obj, property_name):
        """
        Description: This method used to get element ::before style properties
        ...
        property_value = self._driver.execute_script(script, element_obj)
        return property_value
    ...
    def get_element_all_attributes(self, element_obj):
        """
        Description: This function will return all attributes and it's value of object.
        """
        element_attributes_dict = self._driver.execute_script('var items = {}; for (index = 0; index < arguments['
                                                             '0].attributes.length; ++index) { items[arguments['
                                                             '0].attributes[index].name] = arguments[0].attributes['
                                                             'index].value }; return items;', element_obj)
        return element_attributes_dict

    def check_vertical_scroll_reached_bottom(self, scrollable_element_obj):
        """
        Description: This method used to check whether vertical scroll bar reached bottom of the given element object. It will True if vertical scroll reached bottom of the element else False
        """
        script = "return arguments[0].offsetHeight + arguments[0].scrollTop == arguments[0].scrollHeight + (arguments[" \
                 "0].offsetHeight - arguments[0].clientHeight) "
        status = self._driver.execute_script(script, scrollable_element_obj)
        return status

    def check_element_has_vertical_scrollbar(self, scrollable_element_obj):
        """
        Description: This method used to check whether given element has vertical scroll bar. It will True if if element has vertical scroll else False
        """
    ...
    def check_element_has_horizontal_scrollbar(self, scrollable_element_obj):
        """
        Description: This method used to check whether given element has horizontal scroll bar. It will True if if element has horizontal scroll else False
        """
    ...
    def is_image_loaded_in_img_tag(self, img_tab_object):
        ...
        status = self._driver.execute_script(script, img_tab_object)
        return status

    def wait_for_page_loads(self, time_out, sleep_interval=0.5, pause_time=1):
        """
        Description: Webdriver will wait until complete the page load
        """
        ...
        while True:
            ...
            if time.time() > end_time:
                msg = "'{0}' page took more than {0} seconds to load".format(self._driver.title, time_out)
                print(msg)
                break
            try:
                page_status = self._driver.execute_script(script)
    ...
    def get_parent_element(self, element_obj):
        """
        Description: Return the parent element object of given element
        """
    ...
    def get_highlighted_text(self):
        """
        Description: Returns all the highlighted text in the page
        """
        script = "return window.getSelection().toString();"
        highlighted_text = self._driver.execute_script(script)
